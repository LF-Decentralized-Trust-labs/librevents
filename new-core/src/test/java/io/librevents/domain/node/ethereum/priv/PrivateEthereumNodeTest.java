...
final class PrivateEthereumNodeTest extends BaseNodeTest {
    ...
    private static final PrecompiledAddress DEFAULT_PRECOMPILED_ADDRESS = new PrecompiledAddress("test-address");

    @Override
    protected Node createNode(UUID id, NodeName name, SubscriptionConfiguration subscriptionConfiguration, InteractionConfiguration interactionConfiguration, NodeConnection connection) {
        return new PrivateEthereumNode(
                id, name, subscriptionConfiguration, interactionConfiguration, connection, DEFAULT_GROUP_ID, DEFAULT_PRECOMPILED_ADDRESS);
    }
